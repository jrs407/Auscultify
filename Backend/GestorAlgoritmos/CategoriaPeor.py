...
class CategoriaPeor:
    def __init__(self):
        self.nombre = "CategoriaPeor"
        self.descripcion = "Te pone automaticamente preguntas de la categoria que peor se te da."
        self.db_config = {
    ...
    def obtener_categoria_peor(self, usuario_id):
        """Obtiene la categoría con peor porcentaje de aciertos para el usuario"""
        try:
            ...
            categoria_peor = cursor.fetchone()
            
            cursor.close()
            connection.close()
            
            return categoria_peor
            
        except mysql.connector.Error as db_error:
            raise Exception(f'Error de base de datos al obtener categoría peor: {str(db_error)}')
    ...
    def ejecutar(self, data):
        try:
            ...
            categoria_peor = self.obtener_categoria_peor(usuario_id)
            
            if not categoria_peor:
                return {
                    'estado': 'error',
                    'mensaje': 'El usuario debe haber respondido al menos 3 preguntas en alguna categoría para usar este algoritmo'
                }
            
            categoria_id = categoria_peor['Categorias_idCategorias']
            ...
            resultado = {
                ...
                'porcentaje_aciertos': round(categoria_peor['porcentaje_aciertos'], 2),
                'total_respondidas_categoria': categoria_peor['total_respondidas'],
                'usuario_id': usuario_id,
                'estado': 'completado',
                'mensaje': f'Algoritmo ejecutado correctamente - {len(preguntas_seleccionadas)} preguntas de la categoría {categoria_id} (peor categoría con {round(categoria_peor["porcentaje_aciertos"], 2)}% de aciertos)'
            }
